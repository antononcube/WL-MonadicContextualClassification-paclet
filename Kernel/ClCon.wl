...
BeginPackage["AntonAntonov`MonadicContextualClassification`ClCon`"];
...
GenerateStateMonadCode["AntonAntonov`MonadicContextualClassification`ClCon", "FailureSymbol" -> $ClConFailure];
...
(* Here we use / copied MathematicaForPredictionUtilities`DataArrayRulesForClassifyQ *)
...
ClConROCPlot[xs_, context_?AssociationQ]:= ClConROCPlot[][xs, context];
...
ClConROCPlot[ xFuncName_String, yFuncName_String, opts : OptionsPattern[]][xs_, context_] :=
    Block[{rocDataOpts, rocPlotOpts, rocPlotFunc, echoQ},
...
ClCon /: MakeBoxes[obj_ClCon, form : StandardForm] :=
    Block[{ctx = ClConBind[obj, ClConTakeContext]},
      BoxForm`ArrangeSummaryBox[
        ClCon, obj,
        None, (*the next argument is the always visible properties*)
        {
          If[KeyExistsQ[ctx, "trainingData"],
            {
              BoxForm`SummaryItem@{"Training data dimensions: ", Dimensions[ClConBind[obj, ClConTakeTrainingData]]}
            },
            (*ELSE*)
            {BoxForm`SummaryItem@{"No training data"}}
          ],
          If[KeyExistsQ[ctx, "testData"],
            {
              BoxForm`SummaryItem@{"Test data dimensions: ", Dimensions[ClConBind[obj, ClConTakeTestData]]}
            },
            (*ELSE*)
            {BoxForm`SummaryItem@{"No test data"}}
          ],
          If[KeyExistsQ[ctx, "classifier"],
            {
              BoxForm`SummaryItem@{"Classifier: ", ClConBind[obj, ClConTakeClassifier]}
            },
            (*ELSE*)
            {BoxForm`SummaryItem@{"No classifier"}}
          ]
        },
        {
          BoxForm`SummaryItem@{"Value: ", Short @ ClConBind[obj, ClConTakeValue]}
        },
        form]
    ];
